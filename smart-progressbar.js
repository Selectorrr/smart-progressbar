...
angular.module('angular-smart-progressbar', [])
        ...
        $provide.decorator('$http', ['$delegate', '_AspPromiseTracker',
            function ($delegate, _AspPromiseTracker) {
                angular.forEach('get post put delete jsonp head patch'.split(' '), function (method) {
                    ...
                    $delegate[method] = function decorated(msg) {
    ...
    .service('_AspPromiseTracker', ['$timeout', '$q', function ($timeout, $q) {
        var tracker = {};
        tracker.promises = [];
        ...
        return tracker;}])
    .run(function ($rootScope, _AspPromiseTracker) {
        ...
        $rootScope.$watch(_AspPromiseTracker.active, function (isActive) {
            if(isActive) {
                progress.css({visibility: 'visible', opacity: '0.5'})
            } else {
